...
def train(timestep):
    ...
    model = Sequential()
    model.add(LSTM(1, return_sequences=True,input_shape=(timestep, 6)))
    # print('Build model...')
    # model = Sequential()
    ...
    #
    print("Train...")
    model.fit(X_train, y_train, batch_size=16, nb_epoch=5, validation_split=0.2,show_accuracy=True)
    score = model.evaluate(X_test, y_test, batch_size=16, show_accuracy=True)
    print('Test score:', score)
    proba = model.predict_classes(X_test[:5], batch_size=32)
    print (proba)
