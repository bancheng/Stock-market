...
"""
########################################################################################
##This file must be placed under the file "mosesdecoder-master" to make it performable##
########################################################################################
...
def tokenize(sentences):
    ...
    print 'Tokenizing..',
    text = "\n".join(sentences)
    tokenizer = Popen(tokenizer_cmd, stdin=PIPE, stdout=PIPE)   ###Popen函数没看懂？？？
...
def build_dict(path):
    ...
    sorted_idx = numpy.argsort(counts)[::-1]   ##索引值降序排列
