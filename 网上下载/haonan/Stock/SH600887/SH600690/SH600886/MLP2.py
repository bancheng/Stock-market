# -*- coding: utf-8 -*-
"""
Created on Tue Feb 28 07:20:34 2017
...
#        self.errors = self.logRegressionLayer.errors
        ...
        # made out of
        #单层MLP参数由内层隐藏层和最后一层softmax输出层组成
        ...
        self.input = input
        #
        self.p_y_given_x = T.nnet.softmax(T.dot(self.hiddenLayer.output, self.logRegressionLayer.W) + self.logRegressionLayer.b)
        #计算输出值
        self.y_pred = T.argmax(self.p_y_given_x,axis=1)
...
# error计算的是错误率
    ...
    def errors(self, y):
     ...
     #定义了MLP的训练过程        
def test_mlp(trainSet,validSet,testSet,trainLabel,validLabel,testLabel,
             learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000, batch_size=10, n_hidden=500):
    """
...
#    start_time = timeit.default_timer()
    ...
    while (epoch < n_epochs) and (not done_looping):
        #和LG相同，每一次epoch相当于对所有数据进行一次遍历
        # 每一次遍历的时候都将数据集分成很多小训练集，在每个小训练集上进行训练权重调节
        epoch = epoch + 1
...
#                           test_score * 100.))
                    # save the best model
                    #pickle 不能保存文件类对象，所以如果MLP中量是引用类外其他函数计算得来那么就无法保存，正确方法
                    #是将所有函数内置在类内
                    with open('best_model_MLPNormal.pkl', 'wb') as f:
...
def predict(test_set_x):
    ...
    classifier = pickle.load(open('best_model_MLPNormal.pkl'))
