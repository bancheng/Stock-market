# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 10:17:30 2017
...
if __name__ == '__main__': 
    ...
    for n_hiddenC1 in [8,7,6,5,4,3]:
            test_SdA(trainSetCC,validSetCC,testSetCC,trainLabelCC,validLabelCC,testLabelCC,
                                                     finetune_lr=0.1, pretraining_epochs=70,corruption_level=[0.2],
                                                     pretrain_lr=0.01, training_epochs=1000, batch_size=10,hidden_layers_sizes=[n_hiddenC1])
            ...
            for  i in range(len(testSetPreCC)):
                if(testSetPreCC[i]!=testLabelCC[i]):
                    Error+=1
            print "n_hiddenC1 %d, rightRate %f "%(n_hiddenC1, (1.0 - float(Error)/len(testLabelCC)))
...
if __name__ == '__main__':
    ...
    for n_hiddenL1 in [8,7,6,5,4,3]:

            for n_hiddenH1 in [8,7,6,5,4,3]:
                
                    test_SdA(trainSetLL,validSetLL,testSetLL,trainLabelLL,validLabelLL,testLabelLL,
                                                     finetune_lr=0.1, pretraining_epochs=70,corruption_level=[0.2],
                                                     pretrain_lr=0.01, training_epochs=1000, batch_size=10,hidden_layers_sizes=[n_hiddenL1])
                    ...
                    test_SdA(trainSetHH,validSetHH,testSetHH,trainLabelHH,validLabelHH,testLabelHH,
                                                     finetune_lr=0.1, pretraining_epochs=70,corruption_level=[0.2],
                                                     pretrain_lr=0.01, training_epochs=1000, batch_size=10,hidden_layers_sizes=[n_hiddenH1])
                    ...
                    print "hiddenL1 %d, hiddenH1 %d, RightRate%f"%(n_hiddenL1, n_hiddenH1, RightRate)
                    if (RightRate>=bestRightRate):
                        bestRightRate = RightRate
                        print "hiddenL1 %d, hiddenH1 %d,bestRightRate%f"%( n_hiddenL1, n_hiddenH1,bestRightRate)
                        Results.append([n_hiddenL1, n_hiddenH1,bestRightRate])
    print Results
                ...
                for k4 in  [3,5,7,9]:
                    ...
                    for n_hiddenL1 in [8,7,6,5,4,3]:
                            ...
                            for n_hiddenH1 in [8,7,6,5,4,3]:
                                            ...
                                            print "k1 %d, k2 %d, k3 %d, k4 %d [n_hiddenL1 %d],[n_hiddenH1 %d]  ,RightRate%f"%(k1,k2,k3,k4, n_hiddenL1, n_hiddenH1,RightRate)
                                            if (RightRate>=bestRightRate):
                                                bestRightRate = RightRate
                                                print "k1 %d, k2 %d, k3 %d, k4 %d ,[n_hiddenL1 %d],[n_hiddenH1 %d] ,bestRightRate%f"%(k1,k2,k3,k4, n_hiddenL1, n_hiddenH1,bestRightRate)
                                                Results.append([k1,k2,k3,k4,[n_hiddenL1],[n_hiddenH1],bestRightRate])
    print Results
