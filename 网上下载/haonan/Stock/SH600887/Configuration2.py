...
import numpy as np 
import operator
from MLP2 import test_mlp,predict
...
if __name__ == '__main__': 
    ...
    for n_hidden in [5,10,30,50,100]:
            test_mlp(trainSetCC,validSetCC,testSetCC,trainLabelCC,validLabelCC,testLabelCC,
                                 learning_rate=0.01, L1_reg=0.01, L2_reg=0.0001, n_epochs=1000, batch_size=10, n_hidden=n_hidden)
            ...
            for  i in range(len(testSetPreCC)):
                if(testSetPreCC[i]!=testLabelCC[i]):
                    Error+=1
            print "n_hidden%d , rightRate %f "%(n_hidden, (1.0 - float(Error)/len(testLabelCC)))
...
if __name__ == '__main__':
    ...
    for n_hidden1 in [5,10,30,50]:
        test_mlp(trainSetLL,validSetLL,testSetLL,trainLabelLL,validLabelLL,testLabelLL,
                                 learning_rate=0.01, L1_reg=0.01, L2_reg=0.0001, n_epochs=1000, batch_size=10, n_hidden=n_hidden1)
        validSetPreLL = predict(validSetLL)
        testSetPreLL = predict(testSetLL)
        for n_hidden2 in [5,10,30,50]:
            
                        
            test_mlp(trainSetHH,validSetHH,testSetHH,trainLabelHH,validLabelHH,testLabelHH,
                                 learning_rate=0.01, L1_reg=0.01, L2_reg=0.0001, n_epochs=1000, batch_size=10,n_hidden= n_hidden2)
            ...
            print "hiddenLL%d ,hiddenHH %d ,RightRate%f"%(n_hidden1, n_hidden2,RightRate)
            if (RightRate>=bestRightRate):
                bestRightRate = RightRate
                print "hiddenLL%d ,hiddenHH %d ,bestRightRate%f"%( n_hidden1, n_hidden2,bestRightRate)
                Results.append([n_hidden1, n_hidden2,bestRightRate])
    print Results
        ...
        for k2 in [3,5,7,9]:
            ...
            for k3 in [3,5,7,9]:
                ...
                for k4 in [3,5,7,9]:
                    ...
                    for n_hidden1 in [5,10,30,50]:
                        test_mlp(newSetLL,validSetLL,testSetLL,newLabelLL,validLabelLL,testLabelLL,
                                 learning_rate=0.01, L1_reg=0.01, L2_reg=0.0001, n_epochs=1000, batch_size=10, n_hidden=n_hidden1)
                        validSetPreLL = predict(validSetLL)
                        testSetPreLL = predict(testSetLL)
                        for n_hidden2 in [5,10,30,50]:
                            ...
                            test_mlp(newSetHH,validSetHH,testSetHH,newLabelHH,validLabelHH,testLabelHH,
                                 learning_rate=0.01, L1_reg=0.01, L2_reg=0.0001, n_epochs=1000, batch_size=10,n_hidden= n_hidden2)
                            validSetPreHH = predict(validSetHH)
                            testSetPreHH = predict(testSetHH)
                            ...
                            print "k1 %d, k2 %d, k3 %d, k4 %d ,hiddenLL%d ,hiddenHH %d ,RightRate%f"%(k1,k2,k3,k4, n_hidden1, n_hidden2,RightRate)
                            if (RightRate>=bestRightRate):
                                bestRightRate = RightRate
                                print "k1 %d, k2 %d, k3 %d, k4 %d ,hiddenLL%d ,hiddenHH %d ,bestRightRate%f"%(k1,k2,k3,k4, n_hidden1, n_hidden2,bestRightRate)
                                Results.append([k1,k2,k3,k4, n_hidden1, n_hidden2,bestRightRate])
    print Results
