...
# 起始始数据采用SZ399001  19990726 diff0 = 23.55 , diff1 = 21.49,dea0 = 64.32
#diff0 = -39.46 , diff1 = -41.20 ,dea0 = -31.0  20050104
def MACD(C,diff0 = -39.46 , diff1 = -41.20 ,dea0 = -31.0):# 此处计算出的MACD 实际为DEA，为DIF的9日移动均值，该处计算为参考论文02中的数据指标
    m = len(C)
    DIFF = np.zeros((m))
    DEA = np.zeros((m))
    EMA12 = np.zeros((m))
...
def featureConstruction(dataSet):
    ...
    DEA,DIFF,MACDBar = MACD(C,diff0 = -39.46 , diff1 = -41.20 ,dea0 = -31.0)
