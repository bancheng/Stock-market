...
# # 起始始数据采用SH000300  2005年1月4日的数据
def MACD(C,diff0 = 0 , diff1 = 0.779,dea0 = 0):# 此处计算出的MACD 实际为DEA，为DIF的9日移动均值，该处计算为参考论文02中的数据指标
    m = len(C)
    DIFF = np.zeros((m))
    DEA = np.zeros((m))
    EMA12 = np.zeros((m))
...
def featureConstruction(dataSet):
    ...
    DEA,DIFF,MACDBar = MACD(C,diff0 = 0 , diff1 = 0.779,dea0 = 0)
