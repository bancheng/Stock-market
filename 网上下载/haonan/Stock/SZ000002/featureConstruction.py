...
def MACD(C,diff0 = 0.065 , diff1 = 0.074,dea0 = 0.056):# 此处计算出的MACD 实际为DEA，为DIF的9日移动均值，该处计算为参考论文02中的数据指标
    m = len(C)
    DIFF = np.zeros((m))
    DEA = np.zeros((m))
    EMA12 = np.zeros((m))
...
def featureConstruction(dataSet):
    ...
    DEA,DIFF,MACDBar = MACD(C,diff0 = 0.037 , diff1 = 0.038,dea0 = 0.039)
